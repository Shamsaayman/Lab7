...
public class StudentController {
    ...
    public ResponseEntity getBatch(@PathVariable String id){
        if(studentService.getBatch(id)==null){
            return ResponseEntity.status(400).body(new ApiResponse("You Already Graduated"));
        }
        else{
            return ResponseEntity.status(200).body(studentService.getBatch(id));
        }
    }

    @GetMapping("isgraduate/{id}")
    public ResponseEntity isGraduate(@PathVariable String id){
        if(studentService.isGraduate(id)){
            return ResponseEntity.status(400).body(new ApiResponse("Invalid Id"));
        }
        else{
            return ResponseEntity.status(200).body(new ApiResponse("Student is a graduate"));
        }
    }
}
