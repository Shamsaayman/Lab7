...
public class TeacherController {
     ...
     @PutMapping("/partime/{id}")
     public ResponseEntity makePartime( @PathVariable int id){

          if( teacherService.makePartime(id)==null){
               return ResponseEntity.status(400).body(new ApiResponse("Id Doesn't Exist"));
          }
          return ResponseEntity.status(200).body(teacherService.makePartime(id));
     }

     @GetMapping("/getpartime")
     public ResponseEntity getPartime(){
          if(teacherService.getPartime()==null){
               return ResponseEntity.status(400).body(new ApiResponse("No Published Articles"));
          }
          return ResponseEntity.status(200).body(teacherService.getPartime());
     }
